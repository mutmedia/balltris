...
function GaussianBlurShader(sigma)
  ...
  for i = -support, support do
    ...
    norm = norm + coef
    fragshader = fragshader..string.format('c += vec4(%f) * Texel(texture, texture_coords + vec2(%f) * offset_direction);\n', coef * 1000, i)
  end

  fragshader = fragshader..string.format('return c * vec4(%f) * color;\n}', norm > 0 and 1/(norm*1000) or 1)


  fragshader = defines..fragshader

  --print('Pixel Shader Generated:\n'..fragshader)

  return love.graphics.newShader(fragshader)
end
