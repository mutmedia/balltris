...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords)
{
  ...
  vec3 color3 = vec3(c);
  color3 *= (1.0 - 0.75 * smoothstep(0.0, time_to_destroy/5.0, delta_time)
      + 0.25 * smoothstep(2.0*time_to_destroy/5.0, 3.0*time_to_destroy/5.0, delta_time)
      - 0.5 * smoothstep(4.0*time_to_destroy/5.0, 5.0*time_to_destroy/5.0, delta_time)
      //- 0.25 * smoothstep(time_destroyed[n] + 0.30, time_destroyed[n] + 0.35, delta_time)
      );

  //color3 *= colorDestroyed;
  return vec4(color3 * vec3(color), c.a);
}
