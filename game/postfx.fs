...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords)
{
  ...
  vec2 point = screen_coords/love_ScreenSize.xy;

  vec3 color3 = vec3(textureColor);
  float h = fract(mix(-0.1, 0.5, point.y) + 1);
  vec3 color_mult = hsv2rgb(vec3(h, 0.3, 0.5));

  vec3 checkered = vec3(1.0);
  if (mod(floor(screen_coords.x/2 + 1000*point.y + 50 * rand(10.0*point.x + point.y)), 2) == 0.0) {
    checkered = vec3(0.90 + 0.08 * rand(10.0*point.x + point.y) * sin(3.0*rand(10.0*point.x + point.y) + 150.0 * point.x));
  }
 if (mod(floor(screen_coords.y/2 + 1000*point.x + 50 * rand(10.0*point.y + point.x)), 2) == 0.0) {
